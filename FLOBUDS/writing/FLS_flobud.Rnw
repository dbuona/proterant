...
lochill<-round(lo.eff[2,1],digits=1)
lolight<-round(lo.eff[3,1],digits=1)
loforce<-round(lo.eff[4,1],digits=1)

lochillXlight<-round(lo.eff[5,1],digits=1)
lochillXforce<-round(lo.eff[6,1],digits=1)
lolightXforce<-round(lo.eff[7,1],digits=1)


chillcomp<-round(bbchill/flochill,digits=1)
lochillcomp<-round(lochill/flochill,digits=1)
loforcecomp<-round(loforce/bbforce,digits=1)
...
lochillforcecomp<-round(lochillXforce/flochillXforce,digits=1)
lochilllightcomp<-round(lochillXlight/flochillXlight,digits=1)
lolightforcecomp<-round(lolightXforce/flolightXforce,digits=1)
...
%EMW24Mar2021: If you keep the re-ordering then move up the definition of FLS... I didn't too fully change this as not sure what you prefer
\noindent Among deciduous woody plants, the relative timing of flower and leaf phenology, or flower-leaf sequences (FLSs), may be particularly consequential to fitness in temperate regions where flowering prior to leaf development is common \citep{Rathcke_1985}. However, the functional significance of FLS variation may depend on other species traits such as pollination syndrome or drought tolerance \citep{Gougherty2018}. \\

%EMW24Mar2021: Whatever we do I suggest we shorten the next three paragraphs -- I think one moderate length paragraph (or two short ones) should be plenty. 
\noindent The flowering-first FLS is strongly correlated with wind-pollination \citep{Buonaiuto2020, Friedman2009}. Models and simulations of pollen movement\linelabel{wind1} show that for wind-pollinated species, flowering-first increases pollen dispersal distances and significantly reduces the amount of pollen interception from non-reproductive structures \citep{Di-Giovanni:1989aa,Tauber1967,Whitehead1969}, suggesting this phenological pattern is critical for the reproductive fitness of wind-pollinated taxa.\\ 
...
However, a proportionate FLS shift in biotically-pollinated taxa may have little impact on fitness, as larger magnitude shifts would need to occur before any significant impact on physiology or pollination success would be\linelabel{wind4} incurred.
...
%EMW24Mar2021: Nice stuff here too!
...
\noindent We fit all models using the R package ``brms" \citep{Burkner2018}. We ran each model on four chains with 4000 iterations and a 3000 iteration warm up for a total of 4000 posterior draws for each parameter. In all models we used weakly informative priors and increasing the priors 5-fold did not affect the model results. We assessed  model performance through ensuring $\hat{R}$s were between 1 and 1.01 and bulk and tail effective sample sizes were high (1800-2800 for most parameters, but as low as  800-900 for some). We present 50\% credible intervals in figures because they are the most computationally stable \citep{Gelman2013}, but provide other intervals in the Supporting Information (Tab. \ref{tab:modelests},Tab. \ref{tab:modelests2}, Tab. \ref{tab:phh}). \\
...
\section*{Results} 
\subsection*{Growth chamber study} 
\noindent  Flowering, leaf budburst and leafout advanced with higher forcing and longer chilling duration (flowering: chilling effect: \Sexpr{flochill}, forcing effect: \Sexpr{floforce}.0, leaf budburst: chilling effect: \Sexpr{bbchill}, forcing effect: \Sexpr{bbforce}, leafout: chilling effect: \Sexpr{lochill}, forcing effect: \Sexpr{loforce}; Fig. \ref{fig:model}, Tab. \ref{tab:modelests}, Tab. \ref{tab:modelests2}---all standardized units; see ``Data analysis" section above), but increases in both of these cues together offset these advances (flowering: force x chill effect: \Sexpr{flochillXforce}.0, leaf budburst: force x chill effect: \Sexpr{bbchillXforce}, leafout: force x chill effect: \Sexpr{lochillXforce}; Fig. \ref{fig:model}, Tab. \ref{tab:modelests}, Tab. \ref{tab:modelests2}). Leaf and flower phenologies diverged in their responses to photoperiod, with flower phenology advancing at longer\linelabel{photo1} photoperiod and leaf phenology  remaining unaffected (leafout) with photoperiod changes, or delaying (budburst) at longer photoperiods when the other two cues were at low levels (Fig. \ref{fig:model}, Tab. \ref{tab:modelests}-\ref{tab:modelests2}). As seen in the interactions between photoperiod and chilling and photoperiod and forcing, increasing chilling or forcing with longer photoperiod advanced the phenology of all three phases. For flowering, leaf budburst and leafout, chilling and forcing were the dominant cues, while photoperiod produced a more muted\linelabel{photo2} phenological response (Fig. \ref{fig:model}, Tab. \ref{tab:modelests}, Tab. \ref{tab:modelests2}). \\

\noindent While leaf and flower phenological responses to environmental cues were qualitatively similar, the strength of their responses to each cue differed substantially. Leaf budburst and leafout responded more strongly to chilling than flowering (budburst: \Sexpr{chillcomp}x, leafout: \Sexpr{lochillcomp}x), and had stronger responses to all cue interactions (leaf budburst: forcing x chilling: \Sexpr{chillforcecomp}x, photoperiod x chilling: \Sexpr{chilllightcomp}x, photoperiod x forcing: \Sexpr{lightforcecomp}x, leafout: forcing x chilling: \Sexpr{lochillforcecomp}x, photoperiod x chilling: \Sexpr{lochilllightcomp}x, photoperiod x forcing: \Sexpr{lolightforcecomp}x; Fig. \ref{fig:model}, Tab. \ref{tab:modelests}, Tab. \ref{tab:modelests2}). Across all species both budburst and flowering displayed a similar magnitude advance with increased forcing, while leafout responded comparatively much stronger (Fig. \ref{fig:model}, Tab. \ref{tab:modelests}, Tab. \ref{tab:modelests2}).\\
...
%Additionally, when comparing the sensitivites of the two vegetative phenophases (budburst and leafout) we found that for all species, the later phase (leafout) was strongly more sensitive to forcing (\Sexpr{loforcecomp}x; Fig. \ref{fig:FHH} as predicted by the PHH.
...
%EMW24Mar2021: Last sentence of above paragraph needs work -- readers have not kept track of 'concurrent' so just spell it out, also hydraulic demand feels a little out of the blue for readers. It's mentioned only in the intro so you need to either skip it here or refresh readers on why it may matter. 
