...
posto2<-posto%>%group_by(category,scenario3) %>%summarise(interphase=abs(mean(leafout,na.rm=TRUE)-mean(flowering,na.rm=TRUE)))
posto3<-posto%>%group_by(category,scenario3) %>%summarise(interphase=abs(mean(budburst,na.rm=TRUE)-mean(flowering,na.rm=TRUE)))

goo2<-posto2 %>%
  arrange(interphase) %>%
  mutate(category = factor(category, levels=c("flowering-first","concurrent","leafing-first"))) %>%
  ggplot()+geom_linerange(aes(0,ymin=.0,ymax=interphase,color=category),position=pd,size=3)+
  geom_text(aes(0,y=interphase+2,color=category,label=round(interphase,digits=1)),position=pd,size=3)+
  ggthemes::theme_few(base_size = 10)+facet_wrap( ~ scenario3,drop=TRUE, scales="free_x",switch="x",ncol=4)+
  theme(
...
png("Plots/posteriorgroups_go.png",width = 8, height= 8,units = 'in',res = 200)
ggpubr::ggarrange(goo2,goo3,goo,nrow=3,ncol=1,heights = c(.25,.25,.8))
dev.off()
