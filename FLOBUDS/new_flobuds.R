...
buddy<-extract_coefs(mod.bb.int)
leafy<-extract_coefs(mod.lo.int)
leafy$phase<-"leaf exapansion"
flowy$phase<-"flowering"
buddy$phase<-"budburst"

allphase<-rbind(flowy,leafy,buddy)

allphase.fect<-filter(allphase,Predictor!="Intercept")
pd=position_dodgev(height=0.4)

jpeg("Plots/flo_buds_figures/fixeffs.jpeg",width=11, height=6,res=300,units = "in")
allphase.fect %>%
  arrange(Estimate) %>%
  mutate(Predictor = factor(Predictor, levels=c("Chill:Light","Chill:Force","Light:Force","Chill","Light","Force","Intercept"))) %>%
  ggplot(aes(Estimate,Predictor))+geom_point(aes(shape=phase,color=phase),position=pd,size=3)+
  ...
  scale_color_manual(values=c("green3","deeppink3","dodgerblue4"))+xlim(-45,40)+
  scale_shape_manual(values=c(15,16,17))+
  theme_linedraw(base_size = 11)+geom_vline(aes(xintercept=0),color="black")
 dev.off() 

####to do: random effects plot with Cats code 

#### 
unique(dat$GEN.SPA)
hysters<-c("COM.PER","COR.COR","ACE.RUB","ACE.SAC","BET.ALL")
