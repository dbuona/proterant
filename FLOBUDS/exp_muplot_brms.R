...
muplotfx <- function(modelhere,modelhere2,modelhere3, nameforfig, width, height, ylim, xlim, leg1, leg2,leg3,leg4){
  ...
  for(i in 1:6){#i=1 ## need to change
    ...
    pos.x3<-modoutput3[(modoutput3$term==rownameshere[i]),"estimate"]
    lines(modoutput3[(modoutput3$term==rownameshere[i]),c("lower","upper")],rep(pos.y,2),col="darkgrey")
    
    points(pos.x,pos.y,cex=1.5,pch=17,col="darkblue")
    points(pos.x2,pos.y,cex=1.5,pch=19,col="darkblue")
    points(pos.x3,pos.y,cex=1.5,pch=15,col="darkblue")    
    values<-c()
      ...
      pos.x.sps.3<-mod.ranef3[pos.sps.i[i],"mean"]
      ...
      lines(mod.ranef2[pos.sps.i[i],c("25%","75%")],rep(pos.y.sps.i,2),
            col=alpha(my.pal[spsi], alphahere))
      lines(mod.ranef3[pos.sps.i[i],c("25%","75%")],rep(pos.y.sps.i,2),
            col=alpha(my.pal[spsi], alphahere))
      ...
      points(pos.x.sps.3,pos.y.sps.i,cex=0.8,pch=15, col=alpha(my.pal[spsi], alphahere))
      }
  ...
  legend(leg3, leg4, legend=leg.txt, pch=c(19,17,15), ### need to change
         cex=.8, bty="n", text.font=3)
