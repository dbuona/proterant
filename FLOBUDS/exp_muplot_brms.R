...
muplotfx <- function(modelhere,modelhere2, nameforfig, width, height, ylim, xlim, leg1, leg2){
  ...
  axis(2, at=1:4, labels=rev(c("Intercept","Chill", "Light", "Force")), las=1) ### Need to change
  abline(v=0, lty=2, col="darkgrey")
  rownameshere <- c("b_Intercept","b_Chill", "b_Light", "b_Force") ### Need to change
  ppeffects <- c("b_Intercept","b_Chill", "b_Light", "b_Force") ### Need to change
  for(i in 1:4){#i=1 ## need to change
    pos.y<-(4:1)[i] ### need to change
  ...
  legend(leg1, leg2, sort(unique(gsub("_", " ", dat$GEN.SPA))), pch=19, ### need to change
