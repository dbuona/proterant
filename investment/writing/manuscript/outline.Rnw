...
In the American plums, the mean PDSI at which all species were found ranged between -0.40 and 0.67. The slope we estimate for PDSI on shifts between FLS categories was weak with high residual variance . It seems unlikely that this aridity at this scale would provide sufficiently selection pressure.

However at the larger genus, while mean PDSI values ranged from -.39 to .28 the effect size was bigger and fit much better.

A fundamental difference between our species and dry decidious is fruit overlap.

This field of inquiry would benefit from more mechanistic experiments.
Do species with contrasting FLS behave differntly in drought (physiologcal measurements, fruit abortion, flower production, etc?) Given we saw so much lability, can aridiy induce increased hysterhanthy?
