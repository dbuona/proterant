...
Our study provides critical insights into the evolution and function of flower-leaf sequences in biotically-pollinated species.  %paves the way for a continued research agenda regarding 
%function of flower-leaf sequences. 
Our findings that hysteranthous flowering is linked to both aridity tolerance and pollination success %increases the urgency for understanding phenological sequences as global change continues to disrupt pollinator services and impact environmental variability. 
...
The model structure is: 
  ...
  $y_{trait} &= \alpha_{sp} +\alpha_{phylo} + \beta_{hyst.index}*X_{hyst.index} + \epsilon$\\
...
$logit(P(Y \leq j)) &= \alpha_{[j]phylo}+ \beta_{PDSI}*X_{PDSI}+\beta_{fruit diameter}*X_{fruit diameter}+\beta_{floral investment}*X_{flowers/inflorescence}+\epsilon$\\
