...
\end{enumerate}
\section*{Abstract}
% EMW: Probably a good time to work on the abstract ... we should start a little more broad -- 'it is not only' sounds like you're jumping in and you might need to introduce phenology more broadly first. Also, below, after `Our inquiry provides three major insights towards a new framework for understanding FLSs' you start with a nice structure of: we found this, it means this. But then you lose it. I would either stick with that structure OR, if you are limited by words, figure out a different way to cover both results and their implications neatly. 
...
\indent A deep inquiry into the nature of this phenological pattern is necessary and particularly timely now because anthropogenic climate change is altering FLSs (Fig. \ref{fig:Figure 1}). For the three European tree species we examined, the number of days between flowering and leafout is increasing as a result of climate change but the rate of change differs among them. These inter-specific differences in FLS sensitivity to climate change may dramatically .  If, as suggested, FLSs are indeed an important component of woody plant fitness, differential sensitivity to climate change will exacerbate fitness differences between species, influencing which species will persist under altered climate conditions.\\
...
% EMW: One general comment -- you use 'we' both for 'we the authors' and 'we the researchers of the world,' which I think is unnecessarily confusing .... I would try to stick with 'we the authors' and avoid using it for the other meaning. 

% EMW: You use plants and species a little too interchangeably here -- I would just switch to species, even if redundant. I made the switch -- see what you think.
...
% EMW: Dan, are my edits to the topic sentence below in line with your general meaning in this paragraph?
\indent Clearly, these definitions should link to the biological significance of different FLS, but different hypotheses suggest different defitions (as we review below). 
% EMW: This is hard to follow. In the last sentence, what the 'first two taxa'? Does it refer to the sentence before? I think that sentence (the first one below) is a bit too complicated for any reader ... you could make it into a conceptual figure if you want (possibly combine with your example of a sequence above) or try to simplify the writing. 
