...
\par Flower-leaf sequences have traditionally been classified into qualitative categories that are almost always defined at the species level. %However, as we will demonstrate using a long term phenological data set from Harvard Forest in Petersham, MA \citep{OKeefe2015}, accurately classifying species into these categories is not a simple task. 
 The terms hysteranthy, protanthy, proteranthy or precocious flowering describe plants that produce flowers before their leaves, while seranthy describes plants whose flowers open as their leaves approach their full size \citep{Lamont2011, Heinig1899}. The term synanthy can be used to decribe species whose flowering period overlaps their leaf development \citep{Lamont2011}, but the boundries between these catagories are not clearly defined, and their use obscures important inter-specific differences.
\par  Take \textit{Betula alleghaniensis} for example: One would be justified in classifying this species as hysteranthous because its flower buds tend to burst before its leaf buds, or as synanthous because its open flowers overlap the beginning of leaf growth (Fig. \ref{fig:Figure 2}).  Neither category is descriptive enough, because no matter the choice, it is clear that the FLS of this species is fundamentally different from both the hysteranthous \textit{Acer rubrum}, whose flowers open weeks before the leaves and  synanthous \textit{Acer pensylvanicum} whose flowers do not open until leaves are well along in their expansion (Fig. \ref{fig:Figure 2}). 



\\ %Furthermore, both the flowering and leaf growth periods consist of several sub-stages making it difficult to fit FLS patterns neatly into these categories.\\
