...
% Try to make the below more exciting, I did a little below but more needed.
 In this paper, we review the existing FLS hypotheses, discuss how the existence of significant intra-specific variation would modify the hypotheses, then examine these redefined hypotheses with four case studies from temperature forest species. We find support for multiple hypotheses, and show that inference regarding the FLS hypotheses is highly sensitive to how FLS's are defined. \\ 
 Together, our review and case studies provide three major insights towards a new conceptual framework for the study of FLS. First, future research should allow for the coincidence of multiple hypotheses which can be achieved both by allowing for overlapping hypotheses in large community models and by testing individual hypotheses in smaller sub-groupings that control for other trait variation. % This last sentence is great! Make sure the reader gets this point in your discussion!
...
\indent A deep inquiry into the nature of this phenological pattern is necessary and timely because anthropogenic climate change is altering FLS's, with variable outcomes for different species (Fig. \ref{fig:Figure 1}). With warming in recent decades all three major European tree species we examined increased the temporal distance between flowering and leaf phenophases. The rate of this change, however, varied strongly between species with one species (\textit{Fraxinus excelsior})  already exceeding its historic range of variability, while others showed more muted responses. Depending on the function of FLS, this differential FLS sensitivity to climate change may have implications for community composition and population demography in the future.\\ % Good last point, can you spell out what you mean more for readers? Explain in a sentence with example or such why you might get a change? 
...
% Can we add a quick 2-4 sentence intro? Here we do X, because ... 
...
This hypothesis suggests that there would be a significant cost to maintaining floral structures during any stage of leaf activity, and as such, it would be expected only species whose flowering occurred before any leaf expansion would gain this drought advantage. This hypothesis predicts that species that are drought tolerant should flower before leafing out, with minimal overlap between the floral and foliate phenophases. Species that are not drought tolerant gain no real advantage from flowering first, so in these species FLS should be random. %%or is it less about drought tolerance and more about living in dry environments. Are these different?
